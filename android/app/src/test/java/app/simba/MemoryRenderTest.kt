...
/** The memory pane, whose add form has never been looked at. */
@RunWith(ParameterizedRobolectricTestRunner::class)
@GraphicsMode(GraphicsMode.Mode.NATIVE)
@Config(sdk = [34], qualifiers = "w411dp-h914dp-xxhdpi", application = android.app.Application::class)
class MemoryRenderTest(private val design: Design) {
    ...
    fun draws() {
        ...
        compose.setContent {
            SimbaTheme(design = design, dark = true) {
                Column(Modifier.fillMaxSize().background(Bg)) { MemoryFixture() }
            }
        }
        ...
        v.draw(Canvas(b))
        File(OUT, "memory-${design.name.lowercase()}.png").outputStream().use {
